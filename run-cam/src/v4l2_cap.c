...
static void print_capabilities(uint32_t caps)
{
    ...
    if (caps & V4L2_CAP_VIDEO_CAPTURE)
        LOG_I("    [✓] VIDEO_CAPTURE    (视频捕获, 单平面)");
    if (caps & V4L2_CAP_VIDEO_CAPTURE_MPLANE)
        LOG_I("    [✓] VIDEO_CAPTURE_MPLANE (视频捕获, 多平面)");
    if (caps & V4L2_CAP_VIDEO_OUTPUT)
        LOG_I("    [✓] VIDEO_OUTPUT     (视频输出)");
    if (caps & V4L2_CAP_STREAMING)
        LOG_I("    [✓] STREAMING        (支持流式I/O, 即mmap/userptr)");
    if (caps & V4L2_CAP_READWRITE)
        LOG_I("    [✓] READWRITE        (支持read/write I/O)");
    if (caps & V4L2_CAP_META_CAPTURE)
        LOG_I("    [✓] META_CAPTURE     (元数据捕获)");
    if (caps & V4L2_CAP_EXT_PIX_FORMAT)
        LOG_I("    [✓] EXT_PIX_FORMAT   (扩展像素格式)");
    if (caps & V4L2_CAP_DEVICE_CAPS)
        LOG_I("    [✓] DEVICE_CAPS      (有设备级能力)");
}
...
int v4l2_enumerate_all(const char *dev_path, v4l2_cap_info_t *info)
{
    ...
    LOG_I("  驱动版本         = %u.%u.%u", (cap.version >> 16) & 0xFF, (cap.version >> 8) & 0xFF, cap.version & 0xFF);
        ...
        if (info->format_count < 16) {
            ...
            while (xioctl(fd, VIDIOC_ENUM_FRAMESIZES, &frmsize) == 0) {
                if (frmsize.type == V4L2_FRMSIZE_TYPE_DISCRETE) {
                    ...
                    if (fi->frame_count < 32) {
                        ...
                        while (xioctl(fd, VIDIOC_ENUM_FRAMEINTERVALS, &frmival) == 0) {
                            if (frmival.type == V4L2_FRMIVAL_TYPE_DISCRETE) {
                                ...
                                LOG_I("      帧率: %u/%u (%.1f fps)", frmival.discrete.denominator,
                                      frmival.discrete.numerator, fps);

                                if (fs->interval_count < 16) {
                                    fs->intervals[fs->interval_count].numerator   = frmival.discrete.numerator;
                                    fs->intervals[fs->interval_count].denominator = frmival.discrete.denominator;
                                    fs->interval_count++;
                                }
                            }
                            frmival.index++;
                    ...
                    LOG_I("    分辨率: 步进范围 %u-%u x %u-%u (步进 %u x %u)", frmsize.stepwise.min_width,
                          frmsize.stepwise.max_width, frmsize.stepwise.min_height, frmsize.stepwise.max_height,
   ...
   */
    ...
    while (xioctl(fd, VIDIOC_QUERYCTRL, &qctrl) == 0) {
        ...
        LOG_I("    类型: %s | 范围: [%d ~ %d] 步长=%d | 默认=%d | 当前=%d", ctrl_type_name(qctrl.type), qctrl.minimum,
              qctrl.maximum, qctrl.step, qctrl.default_value, ctrl.value);

        /* 标志信息 */
        if (qctrl.flags) {
            LOG_I("    标志: 0x%x%s%s%s%s", qctrl.flags,
        ...
        if (qctrl.type == V4L2_CTRL_TYPE_MENU) {
            ...
            qmenu.id = qctrl.id;
            for (qmenu.index = (uint32_t)qctrl.minimum; (int32_t)qmenu.index <= qctrl.maximum; qmenu.index++) {
    ...
    if (xioctl(fd, VIDIOC_G_PARM, &parm) == 0) {
        LOG_I("  当前帧率        = %u/%u (%.1f fps)", parm.parm.capture.timeperframe.denominator,
              parm.parm.capture.timeperframe.numerator,
              (double)parm.parm.capture.timeperframe.denominator / parm.parm.capture.timeperframe.numerator);
    }
    ...
    for (int i = 0; i < info->format_count; i++) {
        ...
        for (int j = 0; j < fi->frame_count; j++) {
            ...
            for (int k = 0; k < fs->interval_count; k++) {
                double fps = (double)fs->intervals[k].denominator / fs->intervals[k].numerator;
...
void v4l2_print_controls(const v4l2_cap_info_t *info)
{
    ...
    for (int i = 0; i < info->ctrl_count; i++) {
        ...
        printf("  [%2d] %-28s 当前=%-6d 范围=[%d ~ %d] 步长=%d 默认=%d  (ID=0x%08x)\n", i + 1, ci->name,
               ci->current_value, ci->minimum, ci->maximum, ci->step, ci->default_value, ci->id);
    }
}
